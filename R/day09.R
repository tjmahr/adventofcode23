...
#'
#' @param x some data
#' @return For Part One, `f09a_oasis_forecast(x)` returns the sum of the
#'   forecasted values. For Part Two, `f09b_oasis_backcast(x)` returns the sum
#'   of the backcasted values.
#' @export
#' @examples
#' f09a_oasis_forecast(example_data_09())
#' f09b_oasis_backcast(example_data_09())
f09a_oasis_forecast <- function(x) {
  x |>
    f09_helper() |>
    list_lapply(vec_last) |>
    lapply(unlist) |>
    lapply(cumsum) |>
    lapply(vec_last) |>
    unlist() |>
...
#' @rdname day09
#' @export
f09b_oasis_backcast <- function(x) {
  cumulative_subtract_from <- function(xs) {
    Reduce(function(x, y) y - x, x = xs,  accumulate = TRUE)
  }

  x |>
    f09_helper() |>
    list_lapply(vec_head, 1) |>
    lapply(unlist) |>
    lapply(cumulative_subtract_from) |>
    lapply(vec_last) |>
    unlist() |>
    sum()
}
...
f09_helper <- function(x) {
  plumb_diffs <- function(xs) {
    l <- vector("list", length(xs))
    order <- 1
    diffs <- xs
    l[[order]] <- diffs
