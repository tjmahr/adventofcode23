...
#'
#' @param x some data
#' @return For Part One, `f07a_calculate_winnings(x)` returns the total
#'   winnings. For Part Two, `f07b_calculate_jk_winnings(x)` returns the total
#'   winnings under joker rules.
#' @export
#' @examples
#' f07a_calculate_winnings(example_data_07())
#' f07b_calculate_jk_winnings(example_data_07())
f07a_calculate_winnings <- function(x) {
  data <- f07_helper(x)
  sum(data$rank * data$bid)
}


#' @rdname day07
#' @export
f07b_calculate_jk_winnings <- function(x) {
  data <- f07_helper(x)
  sum(data$jk_rank * data$bid)
}


f07_helper <- function(x) {
  score_hand_type <- function(x) {
    mapping <- c(
      "5" = 7,
      "41" = 6,
      "32" = 5,
    ...
    x |>
      ...
      lapply(vec_index_into, mapping) |>
      unlist(use.names = FALSE)
  }

  adjust_hand_for_joker <- function(xs) {
    j_replacements <- xs |>
      chr_replace("J", "") |>
      strsplit("") |>
      lapply(table) |>
      lapply(function(xs) names(which.max(xs))) |>
      lapply(function(xs) if(is.null(xs)) "A" else xs) |>
      unlist()

    list_map2(
      xs,
      j_replacements,
      function(x, y) chr_replace(x, "J", y)
    ) |>
      unlist(use.names = FALSE)
  }

  convert_hand_to_number <- function(x) {
    mapping <- vec_set_names(1:14, c(1:9, "T", "J", "Q", "K", "A"))
    x |>
      strsplit("") |>
      lapply(vec_index_into, mapping) |>
      lapply(as.hexmode) |>
  ...
  data <- x |>
    ...
    list_invoke(rbind) |>
    as.data.frame() |>
    vec_set_names(c("hand", "bid"))

  data$bid <- as.numeric(data$bid)
  data$type <- score_hand_type(data$hand)
  data$hex <- convert_hand_to_number(data$hand)
  data$rank <- data$type |>
    paste0(data$hex) |>
    strtoi(16) |>
    rank()

  # Handle joker by adjusting underlying hands and applying normal functions
  data$jk_type <- data$hand |> adjust_hand_for_joker() |> score_hand_type()
  data$jk_hex <- data$hand |> chr_replace("J", "1") |> convert_hand_to_number()
  data$jk_rank <- data$jk_type |>
    paste0(data$jk_hex) |>
    strtoi(16) |>
    rank()
