...
#' @param x some data
#' @param expansion_factor increase the number of empty rows by a factor of this
#'   amount. Defaults to 2 (i.e., double the number of empty rows).
#' @return `f11a_sum_galaxy_distances(x, expansion factor)`
#'   provdes solutions for both parts, returning the sum of distances after
#'   repeating rows.
#' @export
#' @examples
#' f11a_sum_galaxy_distances(example_data_11())
#' f11a_sum_galaxy_distances(example_data_11(), 10)
f11a_sum_galaxy_distances <- function(x, expansion_factor = 2) {
  galaxies <- f11_helper(x, expansion_factor)
  galaxies |>
...
f11_helper <- function(x, expansion_factor = 2) {
  data <- x |>
    strsplit("") |>
    list_invoke(rbind)

  # To double 1, add 2 - 1; to 10x 1, add 10 - 1
  repeat_amount <- expansion_factor - 1
  row_jumps <- apply(data, 1, function(x) all(x == ".")) * repeat_amount
  col_jumps <- apply(data, 2, function(x) all(x == ".")) * repeat_amount
  row_steps <- rep(1, nrow(data)) + row_jumps
  col_steps <- rep(1, ncol(data)) + col_jumps

  starting_indices <- which(data == "#", arr.ind = TRUE)
  starting_rows <- starting_indices[, 1]
  starting_cols <- starting_indices[, 2]
  new_rows <- cumsum(row_steps)[starting_rows]
  new_cols <- cumsum(col_steps)[starting_cols]
