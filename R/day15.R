...
#'
#' @param x some data
#' @return For Part One, `f15a_sum_hashes(x)` returns the sum of the hashes in
#'   the initialization sequence. For Part Two,
#'   `f15b_compute_focusing_power(x)` returns the focusing power.
#' @export
#' @examples
#' f15a_sum_hashes(example_data_15())
#' f15b_compute_focusing_power(example_data_15())
f15a_sum_hashes <- function(x) {
  hash_string <- function(string) {
    string |>
      utf8ToInt() |>
      vec_reduce(hash_step, init = 0)
  }
  hash_step <- function(x, y) {
    (17 * (x + y)) %% 256
  }
...
#' @rdname day15
#' @export
f15b_compute_focusing_power <- function(x) {
  update_state <- function(state, step) {
    # Move the op checks into the functions with guard statements
    f_try_add <- function(state, step) {
      if (step$op != "add") return(state)
      box <- step$target + 1

      if (vec_has_name(state[[box]], step$label)) {
        state[[box]][step$label] <- step$boxed
      } else {
        state[[box]] <- c(state[[box]], step$boxed)
      }
      state
    }
    f_try_drop <- function(state, step) {
      if (step$op != "drop") return(state)
      box <- step$target + 1
      state[[box]] <- state[[box]] |>
        vec_which_name(step$label, negate = TRUE) |>
        vec_index_into(state[[box]])
      state
    }

    state |> f_try_add(step) |> f_try_drop(step)
  }

  parse_step <- function(string) {
    op <- ifelse(grepl("=", string), "add", "drop")
    if (op == "add") {
      parts <- string |> strsplit("=") |> unlist()
      label <- parts[1]
      value <- parts[2] |> as.numeric()

    } else {
      label <- string |> substr(1, nchar(string) - 1)
      value <- NA_real_
    }
    target <- hash_string(label)
    list(
      string = string,
      op = op,
      label = label,
      value = value,
      target = target,
      boxed = value |> vec_set_names(label)
    )
  }
  hash_string <- function(string) {
    string |>
  ...
  show_state <- function(state) {
    format_box <- function(box) {
      sprintf("[%s %s]", names(box), box) |>
        paste0(collapse = " ")
    }
    boxes_i <- state |>
      lengths() |>
      vec_which_value(0, negate = TRUE)
    box_values <- state[boxes_i] |> lapply(format_box) |> unlist()
    lines <- sprintf("Box %s: %s", boxes_i - 1, box_values)
    writeLines(c(lines, ""))
    invisible(state)
  }

  state <- rep(list(numeric(0)), 256)
  steps <- x |>
    strsplit(",") |>
    unlist() |>
    lapply(parse_step)
