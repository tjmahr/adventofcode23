...
#'
#' @param x some data
#' @return For Part One, `f06a_multiply_winning_distances(x)` returns the
#'   product of winning distances. For Part Two,
#'   `f06b_multiply_winning_distances(x)` modifies `x` to match the puzzle
#'   description and calls `f06a_multiply_winning_distances()` on the modified
#'   input.
#' @export
#' @examples
#' f06a_multiply_winning_distances(example_data_06())
#' f06b_multiply_winning_distances(example_data_06())
f06a_multiply_winning_distances <- function(x) {
  data <- f06_helper(x)

  # apply quadratic formula to:
  #   distance(time) = time * (time_limit - time) > longest_distance
  a <- -1
  b <- data$time
  c <- -data$distance
  ...
  # first whole number right of left root, and vice versa
  left <- floor(pmin(r1, r2) + 1)
  right <- ceiling(pmax(r1, r2) - 1)

  # e.g., 5 - 2 = 3, but c(2, 3, 4, 5) is length 4
  possible_wins <- (right - left) + 1
...
#' @rdname day06
#' @export
f06b_multiply_winning_distances <- function(x) {
  new <- x |> chr_replace("\\s+", "") |> chr_replace(":", ": ")
  f06a_multiply_winning_distances(new)
}
