...
#'
#' @param x some data
#' @return For Part One, `f02a_sum_valid_game_ids(x)` returns the sum of valid
#'   game IDs. For Part Two, `f02b_sum_game_powers(x)` returns the sum of the
#'   game powers.
#' @export
#' @examples
#' f02a_sum_valid_game_ids(example_data_02())
#' f02b_sum_game_powers(example_data_02())
f02a_sum_valid_game_ids <- function(x) {
  games <- f02_helper(x)
  make_color_checker <- function(color, limit) {
    function(x) {
      all(x[[color]] <= limit)
    }
  }
  ...
  games |>
    list_filter(make_color_checker("red", 12)) |>
    list_filter(make_color_checker("green", 13)) |>
    list_filter(make_color_checker("blue", 14)) |>
    lapply(getElement, "game") |>
    unlist() |>
    unique() |>
    sum()
}
...
#' @rdname day02
#' @export
f02b_sum_game_powers <- function(x) {
  games <- f02_helper(x)
...
f02_helper <- function(x) {
  # x <- example_data_02()
  parse_marble_count <- function(x) {
    list(
      blue = x |> chr_extract("\\d+ blue"),
      green = x |> chr_extract("\\d+ green"),
      red = x |> chr_extract("\\d+ red")
    ) |>
  ...
  games <- x |>
    strsplit(";") |>
    lapply(parse_marble_count) |>
    list_map2(seq_along(x), function(x, y) {x$game <- y; x})
