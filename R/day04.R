...
#'
#' @param x some data
#' @return For Part One, `f04a_score_cards(x)` returns the score of the cards.
#'   For Part Two, `f04b_count_cards(x)` returns the total number of cards read.
#' @export
#' @examples
#' f04a_score_cards(example_data_04())
#' f04b_count_cards(example_data_04())
f04a_score_cards <- function(x) {
  numbers <- f04_helper(x)
  exponents <- (lengths(numbers$matches) - 1)
  (2 ^ exponents)  |>
    floor() |>
    sum()
}
...
#' @rdname day04
#' @export
f04b_count_cards <- function(x) {
  count_batch <- function(indices, count, cards) {
    if (length(indices) == 0) return(count)

    new_indices <- cards$next_cards[indices] |>
      unlist() |>
      vec_remove_na()

    count_batch(new_indices, count + length(indices), cards)
  }

  # Precompute next steps for each card
  cards <- f04_helper(x)
  indices <- seq_along(x)
  cards$next_cards <- cards$matches |>
    lengths() |>
    lapply(seq_len) |>
    lapply(function(x) if(length(x)) x else NA) |>
    list_map2(indices, function(x, y) x + y)

  count_batch(indices, 0, cards)
}
...
f04_helper <- function(x) {
  ...
  list(winning = winning, in_hand = in_hand, matches = matches)
}
